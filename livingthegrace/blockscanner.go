...
func (bs *BlockScanner) ScanBlockTask() {
	...
	for {
		...
		if currentHash != block.LastHash {
			...
			err := bs.BatchExtractTransactions(uint64(currentHeight), currentHash, int64(block.Timestamp), block.Txns)
			if err != nil {
...
func (bs *BlockScanner) ExtractTransaction(txid string, scanTargetFunc openwallet.BlockScanTargetFunc) ExtractResult {
	...
	result.BlockTime = int64(transaction.Timestamp)
...
func (bs *BlockScanner) scanBlock(height uint64) (*Block, error) {
	...
	err = bs.BatchExtractTransactions(block.Height, block.Hash, int64(block.Timestamp), block.Txns)
	if err != nil {
...
func (bs *BlockScanner) GetBalanceByAddress(address ...string) ([]*openwallet.Balance, error) {
	...
	for _, a := range address {
		...
		if err == nil {
			...
			balance = balance.Shift(-bs.wm.Decimal())
...
func (bs *BlockScanner) RescanFailedRecord() {
	...
	for height, _ := range blockMap {
		...
		err = bs.BatchExtractTransactions(height, block.Hash, int64(block.Timestamp), block.Txns)
		if err != nil {
			bs.wm.Log.Std.Info("block scanner can not extractRechargeRecords; unexpected error: %v", err)
			continue
		}
